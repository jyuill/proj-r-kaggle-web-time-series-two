...
calcSSE <- function(x){
  loessMod <- try(loess(views ~ index, data=main.all.all.ts, span=0.5), silent=T)
...
```{r Loess prediction}
## add index
main.all.all.ts$index <- 1:nrow(main.all.all.ts) 
loess1 <- loess(views ~ index, data=main.all.all.ts, span=0.3)
summary(loess1)
